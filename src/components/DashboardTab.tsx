...
export default function DashboardTab() {
  ...
  const driverStatusBadge = {
    VERIFIED: { Icon: CheckCircle, cls: 'text-emerald-600 font-bold status-pulse', label: t('dashboard.verified') },
    PENDING:  { Icon: Clock,       cls: 'text-amber-600 font-bold status-pulse',   label: t('dashboard.pending') },
    DECLINED: { Icon: XCircle,     cls: 'text-red-500 font-bold status-pulse',     label: t('dashboard.declined') },
  }
  ...
  if (loading) return (
    <div className="space-y-8 pb-4 pt-2 px-2 animate-pulse">
      ...
      <div className="space-y-4">
        <div className="h-28 w-full bg-slate-200/50 rounded-3xl" />
        <div className="h-24 w-full bg-slate-200/50 rounded-3xl" />
      </div>
    </div>
  )
  ...
  return (
    <div className="space-y-8 pb-4 pt-2">
      ...
      <div className="neu-card p-5 mx-2 mt-6 rounded-3xl">
        ...
        <div className="w-full neu-pressed rounded-full h-3 overflow-hidden">
          <div 
            className="bg-emerald-500 h-3 rounded-full transition-all duration-1000 ease-out shadow-[0_0_10px_rgba(16,185,129,0.5)] fluid-bar" 
      ...
      {!dailyTarget && !weeklyTarget && !monthlyTarget && (
        <div className="neu-card p-4 mx-2 text-center text-sm font-bold text-slate-500 flex flex-col gap-2 rounded-3xl border border-blue-100">
      ...
      {(dailyTarget > 0 || weeklyTarget > 0 || monthlyTarget > 0) && (
        <div className="flex justify-center gap-4 px-2 mt-4">
          {dailyTarget > 0 && (
            <div className="text-center bg-cyan-50/50 rounded-3xl py-2 px-3 flex-1 border border-cyan-100">
          ...
          {weeklyTarget > 0 && (
            <div className="text-center bg-blue-50/50 rounded-3xl py-2 px-3 flex-1 border border-blue-100">
          ...
          {monthlyTarget > 0 && (
            <div className="text-center bg-purple-50/50 rounded-3xl py-2 px-3 flex-1 border border-purple-100">
      ...
      <div className="neu-card-blue p-5 space-y-4">
        <p className="text-xs font-black text-blue-200 uppercase tracking-wider text-center drop-shadow-md">Verification Breakdown</p>
        <div className="grid grid-cols-3 gap-4">
          <div className="text-center neu-pressed-blue rounded-3xl py-4">
            <CheckCircle className="w-5 h-5 text-green-300 mx-auto mb-2 drop-shadow-md" />
            <p className="text-2xl font-black text-white drop-shadow-md">{verified}</p>
            <p className="text-[10px] text-blue-200 font-bold uppercase mt-1">{t('dashboard.verified')}</p>
          </div>
          <div className="text-center neu-pressed-blue rounded-3xl py-4">
            <Clock className="w-5 h-5 text-yellow-300 mx-auto mb-2 drop-shadow-md" />
            <p className="text-2xl font-black text-white drop-shadow-md">{pending}</p>
            <p className="text-[10px] text-blue-200 font-bold uppercase mt-1">{t('dashboard.pending')}</p>
          </div>
          <div className="text-center neu-pressed-blue rounded-3xl py-4">
            <XCircle className="w-5 h-5 text-red-300 mx-auto mb-2 drop-shadow-md" />
            <p className="text-2xl font-black text-white drop-shadow-md">{declined}</p>
            <p className="text-[10px] text-blue-200 font-bold uppercase mt-1">{t('dashboard.declined')}</p>
          </div>
        </div>
        {hasEarnings && (
          <div className="neu-pressed-blue text-white p-3 rounded-3xl text-xs font-semibold text-center mt-2 space-y-1">
            <p>Total earnings: <span className="font-black text-white text-sm drop-shadow-md">{stats?.earnings?.toFixed(2)} Birr</span></p>
      ...
      <div className="pt-2">
        ...
        <div className="space-y-4">
          {drivers.slice(0, 15).map(d => {
            ...
            return (
              <div key={d.id} onClick={() => setSelected(d)}
                className="neu-button p-4 rounded-3xl flex justify-between items-center cursor-pointer">
                <div>
          ...
          {drivers.length === 0 && (
            <div className="text-center py-12 text-slate-500 font-bold text-sm neu-pressed rounded-3xl">
        ...
        {selected && (
          <div className="space-y-5 p-2">
            ...
            {[
              ...
              <div key={row.label} className="flex justify-between items-center neu-pressed px-5 py-3 rounded-3xl">
            ...
            {selected.admin_note && (
              <div className="neu-pressed border-none rounded-3xl p-4 text-sm text-red-600 font-bold">
